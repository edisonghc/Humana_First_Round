...
```{r visualization EDA}
## Visualize the age dist.
demo %>% group_by(age) %>% count() %>% arrange(age) %>% ggplot(aes(x=age,y=n))+geom_col()

demo %>% ggpairs(
    demo %>% select(region,rural,is_lowincome,age,chronic_count,gender,mth_out))
